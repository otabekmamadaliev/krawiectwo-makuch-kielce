/* Uslugi Krawieckie Malgorzata Makuch — skrypt tylko tej strony. Bez zależności. */
(function () {
  'use strict';
  ...
  var pasek = document.querySelector('.pas-gorny');
  if (pasek) {
  ...
  if ('IntersectionObserver' in window && !spokojnie) {
    var cele = document.querySelectorAll('.usluga, .o-nas p, .punkty-listy li, .galeria-mini figure, .kontakt-kolumna');
    var io = new IntersectionObserver(function (es) {
      es.forEach(function (en) {
        if (!en.isIntersecting) return;
        en.target.style.transition = 'opacity .6s ease, transform .6s cubic-bezier(.2,.7,.3,1)';
  ...
  /* Dzisiejszy wpis w ciagu godzin. Liczy przegladarka. */
  (function(){
    var i=(new Date().getDay()+6)%7;
    var el=document.querySelector('.dzien-wpis[data-dzien="'+i+'"]');
